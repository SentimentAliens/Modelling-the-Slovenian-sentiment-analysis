...
#############################################
# 0. Convert KKS XML to CSV
#############################################

XML_FILE = "klxSAcorpus_20160224_1001.xml"
CSV_FILE = "kks_opinion_corpus.csv"

# Only convert if CSV doesn't exist or XML is newer
if not os.path.exists(CSV_FILE) or os.path.getmtime(XML_FILE) > os.path.getmtime(CSV_FILE):
    ...
    # Define namespace (from the XML file)
    ns = {"kks": "http://klxsentiment.azurewebsites.net/annotation/annotateddata.xsd"}
    
    # Extract data from XML
    data = []
    for item in root.findall(".//kks:AnnotatedItem", ns):
        # Extract content (text)
        content_elem = item.find("kks:Content", ns)
        content = content_elem.text if content_elem is not None and content_elem.text else ""
        
        # Extract score (sentiment)
        score_elem = item.find("kks:Score", ns)
        score = int(score_elem.text) if score_elem is not None and score_elem.text else 0
        
        # Map score to label: -1 → negative, 0 → neutral, 1 → positive
        if score == 1:
            label = "positive"
        elif score == -1:
            label = "negative"
        ...
        if content.strip():
            data.append({"text": content.strip(), "label": label})
    
    # Create DataFrame and save to CSV
    df_xml = pd.DataFrame(data)
    df_xml.to_csv(CSV_FILE, index=False, encoding="utf-8")
    print(f"Converted {len(data)} items to {CSV_FILE}")
else:
...
MODEL_NAME = "EMBEDDIA/crosloengual-bert"   # Multilingual BERT for Croatian, Slovenian, and English
...
df_clean.to_csv("kks_opinion_corpus_cleaned_crosloengual-bert.csv", index=False)

print("\nSaved cleaned dataset → kks_opinion_corpus_cleaned_crosloengual-bert.csv")